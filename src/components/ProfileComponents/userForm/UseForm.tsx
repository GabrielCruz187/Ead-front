...
export default function UserForm() {
  ...
  useEffect(() => {
    profileService.fetchCurrentUser().then((user) => {
      setFirstName(user.data.currentUser.firstName);
      setLastName(user.data.currentUser.lastName);
      setPhone(user.data.currentUser.phone);
      setEmail(user.data.currentUser.email);
      setInitialEmail(user.data.currentUser.email);
      setCreatedAt(user.data.currentUser.createdAt);
    });
  }, []);
  ...
  const handleUserUpdate = async function (event: FormEvent<HTMLFormElement>) {
    ...
    const { status } = await profileService.userUpdate({
      firstName,
      lastName,
      phone,
      email,
  ...
  return (
    <>
      <Form className={styles.form} onSubmit={(event) => handleUserUpdate(event)}>
